...
function ModerationPage() {
  ...
  const findTarget = (r: (typeof reports)[number]) => {
    ...
    if (r.target === "comment")
      return comments.find((c) => c.id === r.targetId)?.body.slice(0, 80) ?? "(missing)";
          ...
          <div className="space-y-2">
            {reports.map((r) => (
              <div key={r.id} className="flex items-center gap-3 p-3 rounded-xl bg-secondary/40">
                <span className="text-xs uppercase font-semibold text-muted-foreground w-20">
                  {r.target}
                </span>
                ...
                {r.target === "post" && (
                  <button
                    ...
                    className="h-8 px-3 rounded-lg bg-destructive/10 text-destructive text-xs flex items-center gap-1"
                  >
                ...
                {r.target === "comment" && (
                  <button
                    onClick={() => deleteComment(r.targetId)}
                    className="h-8 px-3 rounded-lg bg-destructive/10 text-destructive text-xs flex items-center gap-1"
                  >
                    <Trash2 className="h-3 w-3" /> Remove
                  </button>
        ...
        <div className="space-y-2">
          {users.map((u) => (
            <div key={u.id} className="flex items-center gap-3 p-3 rounded-xl bg-secondary/40">
              <span className="h-8 w-8 grid place-items-center rounded-lg bg-secondary">
                {u.avatar}
              </span>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold">
                  @{u.username}{" "}
                  {u.banned && <span className="text-xs text-destructive ml-1">banned</span>}
                </div>
                <div className="text-xs text-muted-foreground truncate">{u.bio}</div>
              </div>
              <button
                onClick={() => banUser(u.id)}
                className="h-8 px-3 rounded-lg bg-secondary text-xs flex items-center gap-1"
              >
