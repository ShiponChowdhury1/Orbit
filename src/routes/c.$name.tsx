...
function CommunityPage() {
  ...
  const community = useMemo(
    () => allCommunities.find((c) => c.name === name),
    [allCommunities, name],
  );
  const owner = useMemo(
  ...
  const posts = useMemo(
    () => allPosts.filter((p) => community && p.communityId === community.id && !p.deleted),
    [allPosts, community],
  );
    ...
    <div className="space-y-6">
      <div className="glass rounded-3xl overflow-hidden">
        <div
          ...
          style={{
            background: `linear-gradient(135deg, ${community.color}66, ${community.color}11)`,
          }}
        >
        ...
        <div className="p-6 -mt-12 relative">
          <div className="flex items-end gap-4 flex-wrap">
            <div
              className="h-20 w-20 grid place-items-center rounded-2xl text-4xl border-4 border-background"
            ...
            <button
              ...
              className={`h-10 px-5 rounded-xl text-sm font-medium ${joined ? "bg-secondary" : "bg-primary text-primary-foreground"}`}
            >
            ...
            <ol className="space-y-2 text-sm">
              {(community.rules ?? []).map((r, i) => (
                <li key={r.id} className="flex items-start gap-2">
                  ...
                  {isOwner && (
                    <button
                      onClick={() => removeRule(community.id, r.id)}
            ...
            {isOwner && (
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  if (newRule.trim()) {
                    addRule(community.id, { title: newRule.trim() });
                    setNewRule("");
                  }
                }}
              ...
              >
                <input
                  ...
                  maxLength={80}
                  className="flex-1 h-9 px-2 rounded-lg bg-secondary border border-border text-sm"
                />
                <button
                  type="submit"
                  className="h-9 w-9 grid place-items-center rounded-lg bg-primary text-primary-foreground"
                >
