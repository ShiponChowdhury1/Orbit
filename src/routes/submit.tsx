...
function Submit() {
    ...
    return (
      <div className="glass rounded-2xl p-8 text-center space-y-3">
        ...
        <div className="flex justify-center gap-2">
          <Link
            to="/login"
            className="h-10 px-4 rounded-xl bg-primary text-primary-foreground text-sm font-medium grid place-items-center"
          >
            Sign in
          </Link>
          <Link
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const flair = flairLabel.trim()
      ? { label: flairLabel.trim().slice(0, 24), color: flairColor }
      : undefined;
    const p = create({
      communityId,
      title: title.trim(),
      ...
      <form onSubmit={submit} className="glass rounded-2xl p-6 space-y-5">
        ...
        <div>
          ...
          <div className="mt-1">
            <RichTextEditor
              value={body}
              onChange={setBody}
              placeholder="Share your thoughts — use formatting, links, emojis…"
              minHeight={180}
            />
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              value={flairLabel}
              onChange={(e) => setFlairLabel(e.target.value)}
            ...
            <div className="flex gap-1 mt-2">
              {FLAIR_COLORS.map((c) => (
                <button
                  ...
                  className={`h-7 w-7 rounded-md ${flairColor === c ? "ring-2 ring-foreground" : ""}`}
                  style={{ background: c }}
                />
              ))}
            </div>
        ...
        <div className="flex gap-2">
          ...
          <Link
            to="/"
            className="h-11 px-5 rounded-xl bg-secondary font-medium grid place-items-center"
          >
            Cancel
          </Link>
        </div>
