    ...
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={submit} className="glass rounded-2xl p-6 space-y-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <input
              ...
              placeholder="design"
              className="flex-1 h-11 px-3 rounded-xl bg-secondary border border-border"
            />
          </div>
        </div>
        <div>
            ...
            <div className="flex flex-wrap gap-1 mt-1">
              {icons.map((i) => (
                <button
                  ...
                  key={i}
                  onClick={() => setIcon(i)}
                  className={`h-10 w-10 rounded-lg text-xl ${icon === i ? "bg-primary/20 ring-2 ring-primary" : "bg-secondary"}`}
                >
                  {i}
                </button>
              ))}
            ...
            <div className="flex flex-wrap gap-1 mt-1">
              {palette.map((p) => (
                <button
                  ...
                  key={p}
                  onClick={() => setColor(p)}
                  className={`h-10 w-10 rounded-lg ${color === p ? "ring-2 ring-foreground" : ""}`}
                  style={{ background: p }}
                />
              ))}
            </div>
        ...
        <label className="flex items-center gap-2 text-sm cursor-pointer">
          <input type="checkbox" checked={nsfw} onChange={(e) => setNsfw(e.target.checked)} /> Mark
          community as NSFW (18+)
        </label>
