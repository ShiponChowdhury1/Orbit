...
export function PostCard({ post }: { post: Post }) {
  ...
  const saved = useMemo(
    () => savedList.some((x) => x.userId === me && x.postId === post.id),
    [savedList, me, post.id],
  );
  const commentsCount = useMemo(
    () => comments.filter((c) => c.postId === post.id && !c.deleted).length,
    [comments, post.id],
  );
    ...
    <article className="glass rounded-2xl p-4 sm:p-5 group hover:border-primary/30 transition">
      <div className="flex items-center gap-2 text-xs text-muted-foreground mb-3 flex-wrap">
        ...
        {community && (
          ...
          >
            <span
              className="h-5 w-5 grid place-items-center rounded-md text-[11px]"
              style={{ background: community.color + "33" }}
            >
              {community.icon}
            </span>
            o/{community.name}
      ...
      <Link to="/post/$id" params={{ id: post.id }} className="block">
        ...
        <h2 className="font-display text-lg sm:text-xl font-semibold leading-tight mb-1.5 group-hover:text-primary transition break-words">
          {post.title}
        </h2>
        {preview && (
          <p className="text-sm text-muted-foreground line-clamp-3 break-words">{preview}</p>
        )}
        {media && (
      ...
      <div className="mt-4 flex items-center gap-1 text-sm text-muted-foreground flex-wrap">
        <div className="flex items-center bg-secondary/60 rounded-full">
          <button
            ...
            className={`h-8 w-8 grid place-items-center rounded-full transition ${my === 1 ? "text-primary" : "hover:text-foreground"}`}
            aria-label="Upvote"
          >
            <ArrowBigUp className="h-4 w-4" fill={my === 1 ? "currentColor" : "none"} />
          </button>
          <span
            className={`text-sm font-semibold tabular-nums w-8 text-center ${my === 1 ? "text-primary" : my === -1 ? "text-destructive" : "text-foreground"}`}
          >
            {score}
          </span>
          <button
            onClick={() => vote("post", post.id, my === -1 ? 0 : -1)}
            className={`h-8 w-8 grid place-items-center rounded-full transition ${my === -1 ? "text-destructive" : "hover:text-foreground"}`}
            aria-label="Downvote"
          >
        ...
        <Link
          to="/post/$id"
          params={{ id: post.id }}
          className="flex items-center gap-1.5 px-3 h-8 rounded-full hover:bg-secondary transition"
        >
          <MessageSquare className="h-4 w-4" /> {commentsCount}
        </Link>
        <button
          onClick={() => toggleSave(post.id)}
          className={`flex items-center gap-1.5 px-3 h-8 rounded-full hover:bg-secondary transition ${saved ? "text-primary" : ""}`}
        >
        ...
        <button
          onClick={() => {
            reportItem("post", post.id, "User report");
          }}
          className="ml-auto flex items-center gap-1.5 px-3 h-8 rounded-full hover:bg-secondary transition"
          aria-label="Report"
        >
